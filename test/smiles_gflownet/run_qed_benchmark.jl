# QED PMO Benchmark: Compare Novel Features vs Baseline
#
# Configurations:
# 1. Baseline (TB v10): fixed β=8.0, reward-rank replay
# 2. β-Schedule: β: 0→8 linear ramp (Direction A)
# 3. |δ|-Replay: rank by |TB error| (Direction B)
# 4. Combined: β-schedule + |δ|-replay (A+B)
...
# Include Oracle infrastructure (required by PMO benchmark)
...
println("=" ^ 70)
println("QED PMO BENCHMARK: Comparing Novel Features vs Baseline")
println("=" ^ 70)
println()
...
# Final comparison
println("\n\n" * "=" ^ 70)
println("FINAL COMPARISON: QED PMO (10K budget)")
println("=" ^ 70)
println()
...
println("Our previous best (TB v10): 0.9355")
println("Our previous RWMLE: 0.9373")
...
serialize(joinpath(@__DIR__, "..", "..", "checkpoints", "novel_directions", "qed_comparison.jls"), results)
println("Results saved to checkpoints/novel_directions/qed_comparison.jls")
flush(stdout)
