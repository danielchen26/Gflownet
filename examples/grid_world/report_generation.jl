...
# Optional dependencies for enhanced reporting (will gracefully degrade if not available)
HAS_PLOTTING = false
HAS_CSV = false
try
    ...
    println("📋 Note: Optional dependencies (Plots, DataFrames, CSV) not available - using basic reporting")
end
...
function generate_comprehensive_results(training_history, eval_trajectories, eval_rewards)
    ...
    if !isempty(eval_trajectories) && HAS_PLOTTING
        create_grid_plot(eval_trajectories, timestamp)
    end
    if !isempty(eval_rewards) && HAS_PLOTTING
...
function create_reward_plot(rewards, timestamp)
    if !HAS_PLOTTING
        println("   Skipping reward plot - Plots.jl not available")
        return false
    end
...
function save_csv_files(eval_trajectories, eval_rewards, training_history)
    if !HAS_CSV
        println("   Skipping CSV files - CSV.jl not available")
        return
    end
