#!/usr/bin/env julia
# TB Fine-Tuning with Detached-END v5: Extended v1 Pattern (12 segments)
#
# v1 results were PROMISING:
#   - Validity PRESERVED (87-93%) over 100 iters (4 × 25-iter segments)
#   - QED trending upward: 0.692 → 0.707 → 0.671 → 0.744
#   - QED at iter 100 was only -0.019 below baseline
#
# v5 strategy: replicate v1's proven pattern for 300 total iterations:
#   - 12 × 25-iter segments (same segment size as v1)
#   - Adam optimizer reset between segments (prevents aggressive momentum)
#   - log_Z re-estimated between segments (keeps TB loss calibrated)
#   - Evaluate every 50 iters (every 2 segments)
#   - Early stopping if validity drops below 60%
...
println("=" ^ 70)
println("TB Fine-Tuning with Detached-END v5 (Extended v1, 12×25 segments)")
println("=" ^ 70)
flush(stdout)
...
# =============================================================================
# TB Fine-Tuning v5: 12 × 25-iter segments
# =============================================================================
println("\n" * "=" ^ 70)
println("TB FINE-TUNING v5 (Detached-END, 12×25 segments, 300 total iters)")
println("=" ^ 70)

ref_params = deepcopy(pretrained.params)
ref_states = deepcopy(init_states)

# Same conservative hyperparameters as v1
seg_config = FinetuningConfig(;
    n_iterations=25,            # Short segments — proven to work in v1
    sample_batch_size=32,
    learning_rate=1e-5,         # Conservative
    gradient_clip_norm=1.0,
    kl_weight=1.0,              # Constant (cosine→0 causes collapse)
    ...
    temperature=1.0,
    epsilon=0.05,
    log_frequency=5,            # Log every 5 iters for real-time monitoring
    reward_exponent=4.0,
    min_reward=0.01,
    training_mode=:tb
)
...
println("  Evaluation: every 50 iters (every 2 segments)")
flush(stdout)
...
for seg in 1:n_segments
    ...
    # Evaluate every 2 segments (every 50 iters) and at key milestones
    if seg % 2 == 0 || seg == 1 || seg == n_segments
        ...
        print_eval("tb_v5_iter$total_iter", stats)
        ...
        # Save checkpoint
        checkpoint_dir = "checkpoints/finetune_tb_v5"
        mkpath(checkpoint_dir)
        open("$checkpoint_dir/tb_v5_iter$(total_iter).jls", "w") do f
        ...
        println("  Saved: $checkpoint_dir/tb_v5_iter$(total_iter).jls")
        flush(stdout)

        # Early stopping: catastrophic validity collapse
...
# =============================================================================
println("\n" * "=" ^ 70)
println("SUMMARY: TB Fine-Tuning v5 (Extended v1 Pattern)")
println("=" ^ 70)
...
for iter in sort(collect(keys(checkpoint_results)))
    ...
    @printf("  %-30s  %5.1f%%  %5d   %6.3f   %5d  %5d  %5d  (delta=%+.3f)\n",
        "TB v5 iter $iter", s.validity, s.n_unique,
        s.mean_qed, s.geq_07, s.geq_08, s.geq_09, delta)
end

println("\nBest checkpoint: iter $best_iter, QED $(round(best_qed, digits=3))")
...
println("  TB v3 iter 100:    QED 0.571 (COLLAPSED)")

if !isempty(checkpoint_results)
    best_s = checkpoint_results[best_iter]
    @printf("  TB v5 iter %d:    QED %.3f (%+.1f%%), validity %.1f%%, >=0.9: %d\n",
        best_iter, best_s.mean_qed,
        100.0 * (best_s.mean_qed - baseline.mean_qed) / baseline.mean_qed,
        best_s.validity, best_s.geq_09)
end
