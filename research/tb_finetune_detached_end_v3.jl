#!/usr/bin/env julia
# TB Fine-Tuning with Detached-END v3: Extended Training (Conservative)
#
# v1 results: validity preserved (87-93%), QED 0.744 at iter 100 (trending up)
# v2 results: COLLAPSED — LR=3e-5 + KL=0.5 too aggressive
#
# v3: Keep v1's conservative hyperparameters (LR=1e-5, KL=1.0) but:
#   - Larger segments (100 iters each) to preserve Adam momentum
#   - 300 total iterations to let TB converge
...
println("=" ^ 70)
println("TB Fine-Tuning with Detached-END v3 (Extended, Conservative)")
println("=" ^ 70)
...
# =============================================================================
# TB Fine-Tuning with Detached-END v3
# =============================================================================
println("\n" * "=" ^ 70)
println("TB FINE-TUNING v3 (Detached-END, LR=1e-5, KL=1.0, 300 iters)")
println("=" ^ 70)

ref_params = deepcopy(pretrained.params)
ref_states = deepcopy(init_states)

# Same hyperparameters as v1 (which worked)
base_config = FinetuningConfig(;
    n_iterations=100,           # Per segment (preserves Adam momentum)
    sample_batch_size=32,
    learning_rate=1e-5,         # Conservative (same as v1)
    gradient_clip_norm=1.0,
    kl_weight=1.0,              # Conservative (same as v1)
...
for (seg_idx, target_iter) in enumerate([100, 200, 300])
    ...
    print_eval("tb_v3_iter$target_iter", stats)
    ...
    # Save checkpoint
    checkpoint_dir = "checkpoints/finetune_tb_detached_v3"
    mkpath(checkpoint_dir)
    open("$checkpoint_dir/tb_v3_iter$(target_iter).jls", "w") do f
    ...
    println("  Saved: $checkpoint_dir/tb_v3_iter$(target_iter).jls")
end
...
# =============================================================================
println("\n" * "=" ^ 70)
println("SUMMARY: TB Fine-Tuning v3 (Detached-END, Extended, Conservative)")
println("=" ^ 70)
...
for iter in sort(collect(keys(checkpoint_results)))
    ...
    @printf("  %-30s  %5.1f%%  %5d   %6.3f   %5d  %5d  %5d  (delta=%+.3f)\n",
        "TB v3 iter $iter", s.validity, s.n_unique,
        s.mean_qed, s.geq_07, s.geq_08, s.geq_09, delta)
end

# Also compare against RWMLE and v1
println("\nContext from earlier experiments:")
println("  RWMLE iter 50:  QED 0.823, validity 92.5%, >=0.9: 34")
println("  TB v1 iter 100: QED 0.744, validity 87.5%, >=0.9: 9")
