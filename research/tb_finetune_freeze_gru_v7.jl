#!/usr/bin/env julia
# TB Fine-Tuning v7: Freeze GRU, Output-Layer-Only Fine-Tuning
#
# ROOT CAUSE (confirmed by v5/v6 experiments):
# TB gradient updates GRU weights → corrupts SMILES grammar → QED degrades.
# Detached-END preserves END timing but atom selection grammar still corrupts.
# Constructive-only delays collapse but doesn't prevent it.
#
# FIX: Freeze GRU layers entirely. Only fine-tune the output MLP (512→512→140).
# The GRU hidden states encode SMILES grammar — keep them frozen.
# The output MLP maps hidden states to token logits — it has enough capacity
# to shift the distribution toward high-QED molecules.
#
# Additional improvements:
# - constructive_only=true (skip destructive gradient)
# - β=8.0 (strong focus on top QED molecules, vs β=4.0 in v5/v6)
# - Higher LR=5e-5 for output layer (fewer params, can learn faster)
# - KL=0.5 (less conservative since GRU is frozen — grammar safe)
...
println("=" ^ 70)
println("TB Fine-Tuning v7: Freeze GRU + Output-Layer-Only")
println("=" ^ 70)
flush(stdout)
...
function count_params(x)
    ...
    elseif x isa NamedTuple
        return sum(count_params(v) for v in values(x))
    else
        return 0
    end
end
...
println("  Output layer params: $(output_p) ($(round(100.0*output_p/total_p, digits=1))% of total)")
println("  Frozen params: $(total_p - output_p) (GRU + embedding)")
...
# =============================================================================
# v7: Freeze GRU + Output-Layer-Only TB Fine-Tuning
# =============================================================================
println("\n" * "=" ^ 70)
println("v7: FREEZE GRU + CONSTRUCTIVE-ONLY TB (12×25 segments)")
println("=" ^ 70)
...
seg_config = FinetuningConfig(;
    ...
    learning_rate=5e-5,          # Higher LR for output layer (fewer params to update)
    gradient_clip_norm=1.0,
    kl_weight=0.5,               # Lower KL: GRU frozen → grammar safe → less regularization needed
    ...
    epsilon=0.05,
    log_frequency=5,
    reward_exponent=8.0,         # Higher β: stronger focus on top QED molecules
    min_reward=0.01,
    training_mode=:tb,
    constructive_only=true,      # Only constructive gradient
    freeze_gru=true              # THE KEY: freeze GRU, only update output MLP
)

println("  LR=$(seg_config.learning_rate)  (5× higher — output layer only)")
println("  KL=$(seg_config.kl_weight)  (lower — GRU frozen = grammar safe)")
println("  beta=$(seg_config.reward_exponent)  (2× higher — focus on top QED)")
println("  DETACHED-END: YES")
println("  CONSTRUCTIVE-ONLY: YES")
println("  FREEZE-GRU: YES  ← key fix in v7")
...
for seg in 1:n_segments
    ...
    if seg % 2 == 0 || seg == 1 || seg == n_segments
        ...
        print_eval("v7_iter$total_iter", stats)
        ...
        # Save checkpoint
        checkpoint_dir = "checkpoints/finetune_tb_v7"
        mkpath(checkpoint_dir)
        open("$checkpoint_dir/tb_v7_iter$(total_iter).jls", "w") do f
...
if best_params !== nothing
    checkpoint_dir = "checkpoints/finetune_tb_v7"
    mkpath(checkpoint_dir)
    open("$checkpoint_dir/tb_v7_best.jls", "w") do f
        Serialization.serialize(f, Dict(
            "params" => best_params,
            "iter" => best_iter,
            "mean_qed" => best_qed
...
# =============================================================================
println("\n" * "=" ^ 70)
println("SUMMARY: v7 Freeze GRU + Constructive-Only TB")
println("=" ^ 70)
...
for iter in sort(collect(keys(checkpoint_results)))
    ...
    @printf("  %-30s  %5.1f%%  %5d   %6.3f   %5d  %5d  %5d  (delta=%+.3f)\n",
        "v7 iter $iter", s.validity, s.n_unique,
...
println("  TB v5  iter 50:    QED 0.742 (-2%), validity 88.5%  (standard TB → collapsed at iter 150)")
println("  TB v6  iter 100:   QED 0.732 (-4%), validity 85.5%  (constructive-only → collapsed at iter 150)")
if best_iter > 0 && !isempty(checkpoint_results)
    bs = checkpoint_results[best_iter]
    @printf("  TB v7  iter %d:  QED %.3f (%+.1f%%), validity %.1f%%  (FREEZE GRU + constructive)\n",
        best_iter, bs.mean_qed, 100.0*(bs.mean_qed - baseline.mean_qed)/baseline.mean_qed, bs.validity)
end
